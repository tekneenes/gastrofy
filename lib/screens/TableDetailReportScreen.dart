...
class TableDetailReportScreen extends StatefulWidget {
  final TableSaleSummary summary;
  final DateTime startDate;
  final DateTime endDate;

  const TableDetailReportScreen({
    super.key,
  ...
  @override
  State<TableDetailReportScreen> createState() => _TableDetailReportScreenState();
}

class _TableDetailReportScreenState extends State<TableDetailReportScreen> {
  ...
  void initState() {
    super.initState();
    _reportFuture = Provider.of<TableReportProvider>(context, listen: false)
        .loadDetailedTableReport(
      tableId: widget.summary.id,
      tableName: widget.summary.name,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder(
        future: _reportFuture ??= Provider.of<TableReportProvider>(context, listen: false)
            .loadDetailedTableReport(
          tableId: widget.summary.id,
          tableName: widget.summary.name,
          startDate: widget.startDate,
        ...
        builder: (context, snapshot) {
          ...
          final analysis = provider.currentTableAnalysis;
          ...
          return ListView(
            ...
            children: [
              ...
              const Text('Saatlik Yoğunluk',
                  style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              _buildHourlyChart(analysis.hourlyDistribution),
              const SizedBox(height: 24),
              const Text('Masada Satılan Ürünler',
                  style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              _buildProductDistributionList(analysis.productDistribution),
  ...
  Widget _buildMetricGrid(TableDetailAnalysis analysis) {
    return GridView.count(
      ...
      children: [
        _buildMiniStat('Toplam Ciro',
            '${NumberFormat('#,##0.00', 'tr_TR').format(widget.summary.totalRevenue)} ₺',
            Colors.blue),
        _buildMiniStat('Sipariş Sayısı', widget.summary.orderCount.toString(), Colors.purple),
  ...
  Widget _buildProductDistributionList(Map<String, int> distribution) {
                ...
                child: Stack(
                  ...
                  children: [
                    PieChart(
                      PieChartData(
                        sectionsSpace: 4,
                        centerSpaceRadius: 100, // Biraz daraltıldı çünkü ekran daha dar
          ...
          child: AspectRatio(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return _buildProductListItem(entry.key, entry.value, color);
              },
            ),
          ),
        ),
  ...
  Widget _buildProductListItem(String name, int quantity, Color color) {
    return Container(
